...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Hide app loading indicator if user loading is done OR if there's a user error (which will show its own overlay)
    if (appLoadingIndicator) {
      if (!isUserLoading || userError) {
        appLoadingIndicator.style.display = 'none';
      } else {
        appLoadingIndicator.style.display = 'flex'; // Keep it visible if still loading and no error
      }
    }
    
    // Show Telegram SDK error overlay if there is a userError
    if (userError && errorOverlay && errorMessageElement) {
      errorMessageElement.textContent = userError;
      errorOverlay.style.display = 'flex';
      if(appLoadingIndicator) appLoadingIndicator.style.display = 'none'; // Hide main loader if SDK error shown
  ...
  const ProtectedAdminRoute: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    if (isCheckingAdmin) {
      // The main loading indicator logic in useEffect should handle this
      // Or, if specifically for this route:
  ...
  if (isUserLoading || isCheckingAdmin) {
    // Rely on the #app-loading-indicator in index.html
    // It's styled to cover the page and will be hidden by the useEffect above when loading finishes.
    return null; 
  }
